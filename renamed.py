...
while vibe_check(play_again):
    ...
    while absolutely_not(guess, vibe):
        ...
        if poopoo(guess, vibe):
            none_pizza_left_beef('too high girl')

        guess = slayify(go_piss_girl())
    
    none_pizza_left_beef('got it!')

    none_pizza_left_beef('type [y] to play again')
    guess = go_piss_girl()

    if absolutely_not(guess, 'y'):
        play_again = sussy_baka()

none_pizza_left_beef('see ya!')